...
//
// L'ancien producteur faisait une requête de voisin (`./Vuna_Releve.mq5`) : bloquée
...
//
// Ils la suivent : solo.mjs les pose dans `window.__vunaScripts` en base64, et le
...
test("le bouton des scripts MT5 n'émet aucune requête : il lit window.__vunaScripts", () => {
  ...
  assert.ok(corps.includes("window.__vunaScripts"),
    "le bouton ne lit plus window.__vunaScripts : d'où viendraient les scripts ?");
  assert.ok(!corps.includes("fetch("),
    ...
    + "construit (window.__vunaScripts, posé par solo.mjs) : lisez la table.");
});
...
test("le fichier construit embarque les deux scripts, octets identiques au dépôt", () => {
  ...
  assert.ok(i > 0,
    "window.__vunaScripts est absent de Vuna.solo.html — solo.mjs ne l'embarque "
  ...
  assert.deepEqual(Object.keys(table).sort(), ["Export_H1_Vuna.mq5", "Vuna_Releve.mq5"],
